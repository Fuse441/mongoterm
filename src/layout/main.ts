...
import { connect } from "@/services/mongodb/mongodb.connector";
import { state } from "@/shared/state";
const config = getConfiguration();
import blessed from "neo-blessed";
import { connectionPanel } from "@/panels/connection.panel.js";
import { theme } from "@/config/app.config.js";
//import { closeDropdown, openDropdown } from "@/core/addListener.js";
...
function createDropdown(top: any) {
...
async function fetchDatabases() {
  const admin = state.mongoClient && state.mongoClient.db().admin();
  const dbs = await admin!.listDatabases();
  return dbs.databases.map((d) => d.name);
}

async function fetchCollections(dbName: string) {
  const db = state.mongoClient!.db(dbName);
  const cols = await db.listCollections().toArray();
  return cols.map((c) => c.name);
}
async function fetchQuery(colName: string) {
  const dbName = state.databases[state.selectedDatabaseIndex];
  const datas = await state
